...
- Required tools: Xcode Command Line Tools (`clang` or `cc`, `codesign`,
  `otool` when investigating Mach-O state), POSIX `sh`, `sudo`.
...
4. `install` copies a host binary into the rootfs, patches Apple Silicon arm64e
...
5. `run` forks, redirects child stdout/stderr to a pipe, chroots, `chdir("/")`,
   and executes the requested binary.
6. `run` must pass the dyld shared-cache environment:
   `DYLD_SHARED_CACHE_DIR=<CACHE_DIR>` and `DYLD_SHARED_REGION=private`.
7. `shell` uses `forkpty(3)` so the child has a terminal-like fd set, then
...
- `bootstrap` copies multi-GB dyld shared cache files. First run is slow.
  Later `install` runs reuse the bootstrapped rootfs and are fast.
